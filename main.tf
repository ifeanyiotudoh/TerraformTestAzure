terraform {
  backend "azurerm" {
    resource_group_name  = "NG_App-infra-repo"
...
resource "azurerm_storage_account" "jonnychipzsa" {
  ...
  tags = {
    environment = "ngappinfraenv1"
  }
}
# Create our vNIC for our VM and assign it to our Virtual Machines Subnet
resource "azurerm_network_interface" "vmnic" {
  name                = "ngappinfravm01nic"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
...
resource "azurerm_virtual_machine" "jonnychipzvm01" {
  ...
  storage_os_disk {
    name              = "ngappinfravm01os"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
  os_profile {
    computer_name  = "ngappinfravm01"
    admin_username = "ngappinfra"
